describe "SS grammar", ->
  ...
  it "tokenizes integers", ->
    ...
    expect(tokens[3]).toEqual value: '60', scopes: ['text.pip', 'constant.numeric.integer.pip']

  it "tokenizes boolean true", ->
    {tokens} = grammar.tokenizeLine 'yes = true'
    expect(tokens[3]).toEqual value: 'true', scopes: ['text.pip', 'constant.language.boolean.pip']

  it "tokenizes boolean yes", ->
    {tokens} = grammar.tokenizeLine 'yes = false'
    expect(tokens[3]).toEqual value: 'false', scopes: ['text.pip', 'constant.language.boolean.pip']

  it "tokenizes boolean yes", ->
    {tokens} = grammar.tokenizeLine 'disable_pip_version_check = yes'
    expect(tokens[3]).toEqual value: 'yes', scopes: ['text.pip', 'constant.language.boolean.pip']

  it "tokenizes boolean no", ->
    {tokens} = grammar.tokenizeLine 'disable_pip_version_check = no'
    expect(tokens[3]).toEqual value: 'no', scopes: ['text.pip', 'constant.language.boolean.pip']

  it "tokenizes string values", ->
    {tokens} = grammar.tokenizeLine 'format = freeze'
    expect(tokens[0]).toEqual value: 'format ', scopes: ['text.pip', 'keyword.other.definition.pip']
    expect(tokens[1]).toEqual value: '=', scopes: ['text.pip', 'punctuation.separator.key-value.pip']
    expect(tokens[3]).toEqual value: 'freeze', scopes: ['text.pip', 'string.unquoted.pip']
